...
class TestV2SerialReturn(FrappeTestCase):
    ...
    def _sale_serial_item(self):
        ...
        return item, customer, frappe.get_doc("Ledgix Sale", sale["sale"])
    ...
    def test_serial_return_context_exposes_original_serial_identity(self):
        item, _customer, sale = self._sale_serial_item()
        context = get_pos_v2_return_context(sale.name)
        row = context["items"][0]
        serials = set(str(row.get("serial_numbers") or "").replace(",", "\n").split())
        self.assertEqual(serials, {"RET-SER-001", "RET-SER-002"})
        self.assertEqual(self._status(item.name, "RET-SER-001"), "Sold")
    ...
    def test_partial_serial_return_restores_exact_selected_serial(self):
        ...
        self.assertEqual(self._status(item.name, "RET-SER-001"), "Available")
        self.assertEqual(self._status(item.name, "RET-SER-002"), "Sold")
        return_doc = frappe.get_doc("Ledgix Sales Return", result["return_id"])
        self.assertEqual(return_doc.items[0].serial_numbers.strip(), "RET-SER-001")
