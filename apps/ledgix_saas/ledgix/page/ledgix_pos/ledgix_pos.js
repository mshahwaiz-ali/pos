...
class LedgixPOSV2 {
			...
			<div class="lx-pos-grid">
				...
				<aside class="lx-cart-panel">
					...
					<div class="lx-cart-footer">
						...
						<div class="lx-summary-row lx-total-row"><span>Total</span><strong class="lx-total">Rs. 0</strong></div><div class="lx-summary-row lx-paid-row"><span>Paid</span><strong class="lx-paid">Rs. 0</strong></div><div class="lx-summary-row lx-remaining-row"><span>Remaining</span><strong class="lx-remaining">Rs. 0</strong></div>
	...
	clear_cart() { this.state.cart = []; this.state.tenders = []; this.state.discount_value = 0; this.state.preview = null; this.render_cart(); }
	...
	render_cart() {
		...
		const preview = this.state.preview || {}; const subtotal = Number(preview.subtotal ?? this.state.cart.reduce((s, r) => s + Number(r.qty) * Number(r.override_rate ?? r.rate), 0)); const total = Number(preview.grand_total ?? subtotal); const paid = this.state.tenders.reduce((s, r) => s + Number(r.amount || 0), 0); const remaining = Math.max(total - paid, 0);
		this.$root.find(".lx-cart-count").text(`${this.state.cart.reduce((s, r) => s + Number(r.qty || 0), 0)} items`); this.$root.find(".lx-subtotal").text(this.money(subtotal)); this.$root.find(".lx-discount-value").text(this.money(preview.discount_amount || 0)); this.$root.find(".lx-tax").text(this.money(preview.tax_amount || 0)); this.$root.find(".lx-total").text(this.money(total)); this.$root.find(".lx-paid").text(this.money(paid)); this.$root.find(".lx-remaining").text(this.money(remaining)); this.render_tenders();
		this.$root.find(".lx-add-payment").prop("disabled", !this.state.cart.length || !this.state.payment_methods.length); this.$root.find(".lx-complete-sale").prop("disabled", !this.state.cart.length || (this.state.sale_channel === "Retail" && remaining > 0.005));
	}
	...
	add_payment() { if(!this.state.cart.length)return; const options=this.state.payment_methods.map(x=>x.name).join("\n"); const due=Math.max(Number(this.state.preview?.grand_total||0)-this.state.tenders.reduce((s,r)=>s+Number(r.amount||0),0),0); frappe.prompt([{fieldname:"payment_method",fieldtype:"Select",label:"Payment Method",options,default:this.state.payment_methods[0]?.name||"",reqd:1},{fieldname:"amount",fieldtype:"Currency",label:"Amount Tendered",default:due,reqd:1},{fieldname:"reference_number",fieldtype:"Data",label:"Reference Number"}],v=>{if(Number(v.amount||0)<=0)return;this.state.tenders.push({payment_method:v.payment_method,amount:Number(v.amount),reference_number:v.reference_number||""});this.render_cart();},"Add Payment","Add"); }
	...
	async complete_sale() {
		...
		try { this.set_loading(true); const result=await this.call("ledgix_saas.api.v2_pos.complete_pos_v2_sale",{cart_items:this.cart_payload(),tenders:this.state.tenders,customer:this.state.customer,sale_channel:this.state.sale_channel,price_list:this.state.price_list,discount_type:this.state.discount_type,discount_value:this.state.discount_value,client_sale_id:(window.crypto&&crypto.randomUUID?crypto.randomUUID():`pos-${Date.now()}-${Math.random()}`)}); frappe.show_alert({message:`Sale ${result.invoice_number||result.sale} completed`,indicator:"green"},5); this.clear_cart(); if(result.sale)this.offer_print(result.sale,result.print_mode); await this.refresh_context(); await this.load_items(); }
	...
	start_return(){frappe.prompt([{fieldname:"sale_name",fieldtype:"Data",label:"Sale / Invoice",reqd:1},{fieldname:"reason",fieldtype:"Small Text",label:"Return Reason",reqd:1}],async v=>{try{const sale=await this.call("ledgix_saas.api.pos.get_pos_sale_for_return",{sale_name:v.sale_name});this.show_return_dialog(sale,v.reason);}catch(e){this.handle_error(e);}},"Return / Refund","Load Sale");}
	show_return_dialog(sale,reason){const rows=sale.items||[];const dialog=new frappe.ui.Dialog({title:`Return ${this.escape(sale.invoice_number||sale.name)}`,size:"large",fields:[{fieldname:"items_html",fieldtype:"HTML"}]});dialog.fields_dict.items_html.$wrapper.html(`<div class="lx-return-list">${rows.map((row,i)=>`<div class="lx-return-row"><div><strong>${this.escape(row.item_name||row.item)}</strong><small>Sold ${row.sold_qty} · Already returned ${row.already_returned_qty||0} · Available ${row.returnable_qty||0}</small></div><input type="number" min="0" max="${Number(row.returnable_qty||0)}" step="1" value="0" data-return-index="${i}"></div>`).join("")}</div>`);dialog.set_primary_action("Create Return",async()=>{const items=[];dialog.$wrapper.find("[data-return-index]").each((_,el)=>{const i=Number($(el).data("return-index"));const qty=Number($(el).val()||0);if(qty>0)items.push({item:rows[i].item,qty});});if(!items.length)return frappe.msgprint("Enter at least one return quantity.");try{const result=await this.call("ledgix_saas.api.pos.create_pos_sales_return",{sale_name:sale.name,return_items:items,reason});dialog.hide();frappe.show_alert({message:`Return ${result.return_name} posted`,indicator:"green"},5);await this.refresh_context();await this.load_items();}catch(e){this.handle_error(e);}});dialog.show();}
